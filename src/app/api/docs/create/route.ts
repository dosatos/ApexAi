...
export async function POST(request: NextRequest) {
  try {
    ...
    // Make request to Python agent to create new document
    const agentUrl = process.env.AGENT_URL || 'http://localhost:9000';
    const response = await fetch(`${agentUrl}/docs/create`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { error: "Internal server error during document creation" },
      { status: 500 }
    );
  }
}
...
export async function GET() {
  return NextResponse.json({ message: "Docs create API endpoint" });
}